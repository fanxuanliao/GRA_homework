...
class myGUI():
    ...
    def rotate(self,event):
        print("rotating")
        newAngle = math.degrees(math.atan2(event.y, event.x) - math.atan2(self.prePos.y, self.prePos.x))  # 旋轉角度公式
    ...
    def stopRotate(self, event):
        print("stop rotate")
        event.widget.unbind("<Motion>")  # Release event
        self.prePos = None  # Reset curIndex
        self.moving_object = []
        self.master.config(cursor="arrow")  # Reset cursor
        globals.data.setBitmap()
